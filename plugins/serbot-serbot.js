...
let handler = async (m, { conn: _conn, args, usedPrefix, command, isOwner }) => {
  ...
  //let authFolderB = m.sender.split('@')[0]
    ...
    if (!fs.existsSync("./GokuBotSession/"+ authFolderB)){
        fs.mkdirSync("./GokuBotSession/"+ authFolderB, { recursive: true });
    }
    args[0] ? fs.writeFileSync("./GokuBotSession/" + authFolderB + "/creds.json", JSON.stringify(JSON.parse(Buffer.from(args[0], "base64").toString("utf-8")), null, '\t')) : ""
    
//--
const {state, saveState, saveCreds} = await useMultiFileAuthState(`./GokuBotSession/${authFolderB}`)
...
async function connectionUpdate(update) {
  ...
  setTimeout(() => {
    ...
    if (args[0]) return
		...
		parent.sendMessage(conn.user.jid, {text : usedPrefix + command + " " + Buffer.from(fs.readFileSync("./GokuBotSession/" + authFolderB + "/creds.json"), "utf-8").toString("base64")}, { quoted: m })
	  }
